...
def align_frame(memory,f_frame,s_frame):

    new_affine = 0

    s_frame = np.array(Image.fromarray(s_frame).convert('L'))
    f_frame = np.array(Image.fromarray(f_frame).convert('L'))
    M_affine = SIFT_algorithm(f_frame, s_frame)
    new_affine = M_affine
    for i in range(len(memory),-1):
        new_affine = compose_matrices(memory[i], new_affine)

    return new_affine


def compose_matrices(M1, M2):
    """
    Args:
        M1, M2:
    ...
    M_total = M2_hom @ M1_hom

    return M_total[:2, :]


def invert_transformation(M):
    """
    הופך טרנספורמציה: אם M היא A→B, תחזיר M_inv שהיא B→A
    """
...
def align_rotation(video):
    ...
    for i in range(1,len(video)):
        # print(i)
        if i == 1:
            memory.append(align_frame([], video[0], video[1]))
        else:
            memory.append(align_frame(memory, video[i-1], video[i]))
        inverse = invert_transformation(memory[-1])
        rotation = np.arctan2(inverse[1, 0], inverse[0, 0])
        print(rotation)
...
def generate_panorama(input_frames_path, n_out_frames):
    ...
    frames = np.array(media.read_video("Example inputs/boat.mp4"))[:50]

    video_rgb = align_rotation(frames)
